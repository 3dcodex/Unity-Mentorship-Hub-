  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-900 font-sans text-gray-900 dark:text-gray-100">
      ...
      <section className="px-6 pt-12 pb-24">
        <div className="max-w-7xl mx-auto bg-blue-50 dark:bg-blue-900/20 rounded-2xl sm:rounded-3xl md:rounded-[40px] p-6 sm:p-4 sm:p-6 md:p-8 md:p-6 sm:p-8 md:p-12 lg:p-24 relative overflow-hidden animate-in fade-in duration-700">
          <div className="max-w-2xl relative z-10 space-y-6">
            <h1 className="text-2xl sm:text-xl sm:text-2xl md:text-xl sm:text-2xl md:text-3xl lg:text-4xl md:text-2xl sm:text-3xl md:text-5xl font-black text-gray-900 dark:text-white">
              Who We Serve: <br/>
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <button className="px-10 py-4 bg-white dark:bg-slate-800 text-gray-900 dark:text-white font-black rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-primary transition-all">View Resources</button>
            </div>
          </div>
      ...
      {/* Roles Section */}
      <section className="py-8 sm:py-6 sm:py-10 md:py-16 md:py-24 px-6 bg-white dark:bg-slate-900">
        <div className="max-w-7xl mx-auto text-center mb-16 space-y-4">
          <h2 className="text-2xl sm:text-xl sm:text-2xl md:text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black dark:text-white">Empowering Every Role in Our Ecosystem</h2>
          <p className="text-gray-500 dark:text-gray-400 font-medium">Discover the tailored support and unique opportunities available for each member of our community.</p>
        </div>
...
const RoleCard: React.FC<{ icon: string, title: string, desc: string, bullets: string[], btnText: string }> = ({ icon, title, desc, bullets, btnText }) => (
  <div className="bg-white dark:bg-slate-800 p-10 rounded-xl sm:rounded-2xl md:rounded-[32px] border border-gray-50 dark:border-gray-700 shadow-xl shadow-gray-200/40 dark:shadow-black/30 flex flex-col items-start text-left space-y-6 group hover:shadow-2xl transition-all">
    <div className="size-8 sm:size-7 sm:size-9 md:size-10 md:size-12 bg-blue-50 dark:bg-blue-900/30 rounded-xl flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all">
      <span className="material-symbols-outlined">{icon}</span>
    </div>
    <div className="space-y-3 flex-1">
      <h3 className="text-base sm:text-base sm:text-lg md:text-xl font-black text-gray-900 dark:text-white">{title}</h3>
    ...
    <button className="w-full py-4 bg-gray-50 dark:bg-slate-700 text-primary dark:text-blue-400 font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-primary hover:text-white transition-all">
      {btnText}
    </button>
  </div>
);
