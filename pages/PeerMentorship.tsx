  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-900">
      ...
      <main className="flex-1 animate-in fade-in duration-700">
        <section className="py-8 sm:py-12 md:py-20 px-6 bg-gradient-to-b from-blue-50 to-white dark:from-blue-900/20 dark:to-slate-900">
          <div className="max-w-4xl mx-auto text-center space-y-4 sm:space-y-6 md:space-y-8">
            <div className="inline-flex px-4 py-2 rounded-full bg-primary/10 dark:bg-primary/20 text-primary dark:text-blue-400 text-[10px] font-black uppercase tracking-widest">Core Pillar</div>
            <h1 className="text-2xl sm:text-3xl md:text-5xl md:text-6xl font-black text-gray-900 dark:text-white leading-tight">Peer Mentorship</h1>
            <p className="text-base sm:text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-400 font-medium leading-relaxed max-w-2xl mx-auto">
        ...
        <section className="py-8 sm:py-6 sm:py-10 md:py-16 md:py-24 px-6">
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-4 sm:gap-8 md:gap-16 items-center">
            <div className="space-y-4 sm:space-y-6 md:space-y-8">
              <h2 className="text-2xl sm:text-xl sm:text-2xl md:text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black text-gray-900 dark:text-white leading-tight">By Students, <br/> For Students</h2>
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-8 -right-8 bg-primary dark:bg-blue-600 p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl md:rounded-[32px] text-white shadow-2xl max-w-xs space-y-2">
        ...
        <section className="py-8 sm:py-6 sm:py-10 md:py-16 md:py-24 bg-gray-900 dark:bg-slate-950 text-white px-6">
        ...
        <section className="py-8 sm:py-16 md:py-32 text-center px-6">
          <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 md:space-y-10">
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-gray-900 dark:text-white">Ready to find your guide?</h2>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <Link to="/who-we-serve" className="px-12 py-5 border-2 border-gray-100 dark:border-gray-700 text-gray-600 dark:text-gray-300 font-bold rounded-2xl hover:bg-gray-50 dark:hover:bg-slate-800 transition-all">
                Learn More
              </Link>
            </div>
          </div>
...
const BenefitItem: React.FC<{ icon: string, title: string, desc: string }> = ({ icon, title, desc }) => (
  <div className="flex gap-6">
    <div className="size-8 sm:size-7 sm:size-9 md:size-10 md:size-12 bg-primary/10 dark:bg-primary/20 rounded-xl flex items-center justify-center text-primary dark:text-blue-400 flex-shrink-0">
      <span className="material-symbols-outlined">{icon}</span>
    </div>
    <div className="space-y-1">
      <h4 className="text-base sm:text-base sm:text-lg md:text-xl font-black text-gray-900 dark:text-white">{title}</h4>
...
const FrameworkCard: React.FC<{ step: string, title: string, desc: string }> = ({ step, title, desc }) => (
  <div className="bg-white/5 dark:bg-white/10 border border-white/10 dark:border-white/20 p-10 rounded-2xl sm:rounded-3xl md:rounded-[40px] text-left space-y-6 hover:bg-white/10 dark:hover:bg-white/15 transition-colors">
    <div className="text-2xl sm:text-3xl md:text-5xl font-black text-primary/30 dark:text-primary/40 leading-none">{step}</div>
    <h3 className="text-base sm:text-lg sm:text-base sm:text-base sm:text-lg md:text-xl md:text-2xl font-black">{title}</h3>
    <p className="text-gray-400 dark:text-gray-300 font-medium leading-relaxed">{desc}</p>
  </div>
);
