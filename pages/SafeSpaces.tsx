  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-900">
      ...
      <main className="flex-1 animate-in fade-in duration-700">
        <section className="py-8 sm:py-12 md:py-20 px-6 bg-gradient-to-b from-green-50 to-white dark:from-green-900/20 dark:to-slate-900 text-center">
          <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 md:space-y-8">
            <div className="inline-flex px-4 py-2 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[10px] font-black uppercase tracking-widest">Trust & Safety</div>
            <h1 className="text-2xl sm:text-3xl md:text-5xl md:text-6xl font-black text-gray-900 dark:text-white leading-tight">Safe Spaces</h1>
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20">
            <div className="space-y-6 sm:space-y-8 md:space-y-12">
              <div className="space-y-4">
                <h2 className="text-2xl sm:text-xl sm:text-2xl md:text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black text-gray-900 dark:text-white">Zero-Tolerance Policy</h2>
            ...
            <div className="bg-gray-900 dark:bg-slate-950 rounded-[48px] p-6 sm:p-4 sm:p-6 md:p-8 md:p-12 text-white space-y-6 sm:space-y-8 md:space-y-10 flex flex-col justify-center">
              <h3 className="text-xl sm:text-2xl md:text-3xl font-black leading-tight">The Unity Safe Seal</h3>
              <p className="text-gray-400 dark:text-gray-300 font-medium text-base sm:text-lg leading-relaxed">
        ...
        <section className="py-8 sm:py-6 sm:py-10 md:py-16 md:py-24 bg-gray-50 dark:bg-slate-800/50 px-6">
          <div className="max-w-4xl mx-auto text-center space-y-4 sm:space-y-6 md:space-y-8">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black dark:text-white">Ready to join our community?</h2>
            <p className="text-gray-500 dark:text-gray-400 font-medium text-base sm:text-lg">By joining, you agree to uphold our Community Standards and champion an environment of mutual respect.</p>
...
const Guideline: React.FC<{ icon: string, title: string, desc: string }> = ({ icon, title, desc }) => (
  <div className="flex gap-6 p-6 rounded-3xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-gray-700 shadow-sm">
    <div className="size-8 sm:size-7 sm:size-9 md:size-10 md:size-12 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-xl flex items-center justify-center flex-shrink-0">
       <span className="material-symbols-outlined">{icon}</span>
    </div>
    <div className="space-y-1">
      <h4 className="text-base sm:text-lg font-black text-gray-900 dark:text-white">{title}</h4>
      <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">{desc}</p>
    </div>
  </div>
);
