...
const Signup: React.FC = () => {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      localStorage.setItem('unity_user_name', user.email ?? 'New Member');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 p-4 sm:p-6">
      ...
      <div className="max-w-4xl w-full bg-white rounded-2xl sm:rounded-3xl md:rounded-[40px] shadow-2xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        {/* Progress Bar */}
        ...
        <div className="p-6 sm:p-8 md:p-12 lg:p-16 space-y-6 sm:space-y-8 md:space-y-12">
          {step === 1 && (
            <div className="space-y-6 sm:space-y-8 md:space-y-12 animate-in fade-in slide-in-from-right-8">
              <div className="text-center space-y-2 sm:space-y-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900">Choose your role</h1>
                <p className="text-sm sm:text-base text-gray-500 font-medium">Join our community in the capacity that fits your journey.</p>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6 sm:space-y-8 md:space-y-12 animate-in fade-in slide-in-from-right-8">
              <div className="text-center space-y-2 sm:space-y-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 leading-tight">Tailor your experience</h1>
                <p className="text-sm sm:text-base text-gray-500 font-medium">This helps us match you with the right peers and mentors.</p>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6 sm:space-y-8 md:space-y-12 animate-in fade-in slide-in-from-right-8 max-w-sm mx-auto">
              <div className="text-center space-y-2 sm:space-y-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900">Final Step</h1>
                <p className="text-sm sm:text-base text-gray-500 font-medium">Secure your account to begin.</p>
              </div>
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Email Address</label>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-gray-50 border-none rounded-2xl p-4 text-sm font-medium focus:ring-4 focus:ring-primary/5 outline-none" placeholder="alex@university.edu" />
                </div>
                <div className="space-y-2">
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-3 pt-6 sm:pt-8 md:pt-12">
            <button 
              ...
              className="w-full sm:w-auto px-6 sm:px-8 py-3 text-xs sm:text-sm font-black text-gray-400 hover:text-gray-600 transition-colors uppercase tracking-widest"
            >
            ...
            <button 
              ...
              className="w-full sm:w-auto bg-primary text-white font-black px-8 sm:px-12 py-3 sm:py-4 rounded-xl sm:rounded-2xl shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm sm:text-base"
            >
              {isLoading ? (
