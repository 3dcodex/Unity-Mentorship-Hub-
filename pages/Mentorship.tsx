...
const Mentorship: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-slate-900">
        <div className="text-center space-y-4">
          ...
          <p className="font-medium text-gray-600 dark:text-gray-400">Loading mentorship...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-6 sm:space-y-8 md:space-y-16 bg-white dark:bg-slate-900 min-h-screen">
      ...
      {/* Matching Quiz CTA */}
      <section className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl sm:rounded-3xl md:rounded-[40px] p-6 sm:p-4 sm:p-6 md:p-8 md:p-12 md:p-16 flex flex-col md:flex-row items-center gap-6 sm:p-4 sm:p-6 md:p-8 md:p-12 border border-blue-100 dark:border-blue-800 shadow-sm relative overflow-hidden group">
        ...
        <div className="flex-1 space-y-4 sm:space-y-6 md:space-y-8 relative z-10">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-gray-900 dark:text-white leading-tight">Take the Mentorship Match Quiz</h2>
...
const TrackCard: React.FC<{ img: string, title: string, desc: string, linkText: string, icon: string, to: string }> = ({ img, title, desc, linkText, icon, to }) => (
  <div className="bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl md:rounded-[40px] border border-gray-50 dark:border-gray-700 shadow-xl shadow-gray-200/40 dark:shadow-black/30 overflow-hidden flex flex-col h-full group">
    ...
    <div className="p-4 sm:p-6 md:p-8 flex flex-col flex-1 space-y-6">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 font-medium leading-relaxed flex-1">{desc}</p>
